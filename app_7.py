...
def to_excel(df):
    ...
    writer.close()  # Substituir 'writer.save()' por 'writer.close()'
...
def main():
    ...
    if (data_file_1 is not None):
        ...
        # Verifique se há dados filtrados antes de calcular a proporção
        if not bank.empty:
            try:
                ...
                st.error(f'Erro ao calcular proporção: {e}')
        else:
            st.warning('Nenhum dado filtrado disponível para plotar.')
